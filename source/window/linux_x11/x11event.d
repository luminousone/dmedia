 ...
 */
module window.linux_x11.x11event;
...
version( Posix ) {
	...
	protected __gshared dml_window[xcb_window_t]	current_x11windows;

	public static bool X11PollEvent( ref dml_event event ) {
        ...
        switch( xevent.response_type & ~0x80 )
        {
            ...
            case XCB_CONFIGURE_NOTIFY:  
            	...
            	auto _window = current_x11windows[notify.window];
            	if( 
            ...
            case XCB_KEY_PRESS:
            	...
            	auto _x11window = cast(dml_x11WindowImpl)_window.impl;
            	if( !((_x11window.lastKeyPressKeycode == kb.detail) && 
            		...
            		event.keyboard(KeyPress, _window, sym0, true );
            	}
            break;
